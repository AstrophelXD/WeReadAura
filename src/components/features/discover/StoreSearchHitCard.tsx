    ...
    <div className="rounded-[var(--radius-sm)] border-[2px] border-[var(--ink)] bg-white p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <h3 className="type-card-title-lg leading-snug">{book.title}</h3>
          <p className="type-card-subtitle mt-1">{book.author}</p>
          <p className="type-caption-muted mt-2">{book.category}</p>
        </div>
      ...
      <p className="type-book-intro font-biao mt-3 line-clamp-3">{book.summary}</p>
      <div className="mt-4 flex flex-wrap gap-2">
        <Button type="button" plain onClick={() => onViewDetails(book)}>
          查看详情
        </Button>
        {onShelf ? (
