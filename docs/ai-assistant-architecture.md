...
- 页面式能力：在首页、统计页、发现页或未来年度报告页中生成阅读人格、偏好、行为、阶段总结等分析模块
...
- `annual-report-service.ts`：生成年报视图所需结构化内容
...
`GET /api/reports/annual` 建议返回结构化年报 DTO。
